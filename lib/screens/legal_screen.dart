...
const List<_Section> _privacy = [
  ...
  _Section(
    ...
    '분석을 위해 문자 내용은 회사 서버로 전송됩니다. 서버는 외부 인공지능 분석에 보내기 전에 '
        '주민등록번호·카드번호·전화번호·계좌번호·이메일 주소를 [PHONE]과 같은 표시로 바꿔 전달합니다.\n\n'
        '다만 문자에 적힌 이름은 가려지지 않으며, 링크 주소는 안전성 검사에 필요하므로 '
        '가리지 않고 그대로 전달합니다. 개인정보가 담긴 링크가 포함된 문자를 검사하실 때는 '
        '이 점을 고려해 주세요.\n\n'
  ...
  _Section(
    ...
    '회사는 개인정보를 제3자에게 판매하지 않습니다. 다만 서비스 제공에 필요한 범위에서 '
        ...
        '· VirusTotal, Google Safe Browsing: 문자에 포함된 링크의 안전성 검사 (링크 주소만 전달)\n'
        '· Google (Firebase): 푸시 알림 전송\n'
        '· Amazon Web Services: 서버 운영과 데이터 보관',
  ),
  _Section(
    '5. 보유 기간',
    '분석 이력은 이용자가 개별 삭제하기 전까지 보관합니다.\n\n'
        '회원 탈퇴 시에는 계정을 탈퇴 상태로 기록해 더 이상 이용할 수 없게 하며, 이때 '
        '회원 정보와 분석 이력이 즉시 파기되지는 않습니다. 즉시 파기를 원하시면 아래 문의 경로로 '
        '요청해 주세요. 관계 법령에서 보존을 요구하는 기록은 해당 기간 동안 보관합니다.',
  ),
  _Section(
    '6. 이용자의 권리',
    '이용자는 언제든지 자신의 개인정보를 조회할 수 있고, 분석 이력을 개별 삭제하거나 '
        '회원 탈퇴를 할 수 있습니다. 마이페이지와 탐지 이력 화면에서 직접 처리할 수 있습니다.\n\n'
        '앱에서 직접 수정할 수 있는 항목은 이름입니다. 그 밖의 정보에 대한 정정·삭제는 '
        '아래 문의 경로로 요청해 주세요.',
  ),
  _Section(
    '7. 문의',
