...
> **범위 제외(2026-08-10)**: 문자 자동 탐지(알림 리스너)·강제 오버레이 경고는 팀 착수 합의가 없어 계획에서 제외했다. 문자 검사는 **사용자가 직접 넣는 수동 분석**이 정식 경로다. `screens/overlay_alert.dart`는 호출되지 않는 잔여 코드.
...
lib/
  ...
  screens/
    ...
    overlay_alert.dart 강제 오버레이 경고(범위 제외 — 호출되지 않는 잔여 코드)
