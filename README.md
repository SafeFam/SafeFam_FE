...
### 남은 작업
- **백엔드 준비됨 · 미연동** — 엔드포인트는 전부 소진(컨트롤러 10개 전수 대조 기준). 다만 `AnalysisResponse.evidenceCards`(SafeFam_BE #101, 사용자 언어로 정리한 위험 근거 최대 5개)는 **응답에 오지만 앱이 아직 그리지 않는다**. 지금은 `indicators`로 위험 근거를 렌더한다.
...
- ⚠️ **재발급 실패를 네트워크 문제와 구분하지 않는다** — `reissue()`가 예외(타임아웃 포함)를 전부 `false`로 뭉개고, `sendAuthorized`는 그걸 받아 세션을 폐기한다. 자동 탐지는 문자 한 통에 3초 예산으로 붙으므로, **액세스 토큰이 만료된 상태에서 문자가 오고 네트워크가 느리면 로그아웃된다**(백그라운드 isolate가 지우는 곳이 공유 저장소라 메인 세션까지 죽는다). 서버가 **거절한** 경우에만 토큰을 버려야 한다.
