...
- 🔴 **BE·AI 수정 대기 2건 — 데모에 그대로 보인다.** 둘 다 운영 서버에서 재현하고 원인까지 확정했다. 상세·수정 방법은 `docs/backend-ai-fix-list.md` **0번·0-2번**.
  - **기간별 통계가 항상 터진다**(BE) — `period=LAST_7_DAYS|LAST_30_DAYS|LAST_90_DAYS`가 전부 실패하고 `ALL`만 된다. 401 '인증이 필요합니다'로 나오지만 실제로는 NPE다. 원인은 `AnalysisRepository`의 `countByRiskLevelSince`·`countByCategorySince`에만 `IS NOT NULL` 가드가 빠져 null 키가 `EnumMap.put`에 들어가는 것. **앱 기본 선택이 7일이라 이력 탭을 열면 항상 깨져 보인다.**
  - **피싱 유형(`category`)이 항상 null이다**(AI) — `result_factory.py:92`가 `phishingType=None`을 하드코딩한다. 이것 하나로 **화면 5곳**이 무너진다: 위 통계 NPE의 직접 원인 · 홈 트렌드 카드 "아직 집계된 트렌드가 없어요" · 이력 유형 필터가 아무것도 못 거름 · 대응 챗봇이 분석 컨텍스트를 한 번도 못 받음 · 결과 제목이 "금융기관 사칭 문자예요"가 아니라 "위험한 문자예요"로 뜸.
- **실기기 검증**(갤럭시 S23 FE · SM-S711N · Android 16 · arm64) — 대부분 끝났다. 캡처는 `docs/shots/`(gitignore).
  - ✅ **첫 실행 검증 완료**(2026-08-25) — 설치·부팅·온보딩 분기까지 정상. 여기서 **권한 팝업 두 개가 온보딩을 덮는 버그**를 찾아 고쳤다(이슈 #132 · PR #133): ①#118의 크래시 방지 호출이 타는 플러그인 메서드 `disableBackgroundService`가 네이티브 권한 요청 경로를 타서, 자동 탐지가 꺼져 있어도 첫 실행에 SMS 권한을 요구했다 ②신규 설치 때 FCM 첫 토큰 발급으로 `onTokenRefresh`가 불려 **로그인 전에** `registerDevice()`가 알림 권한을 요구하고 인증 없는 등록 요청을 보냈다. 둘 다 목 서버·에뮬레이터로는 드러나지 않던 것이다.
...
- **AI 서버 영어 문구**(타팀, 근본 수정은 AI 몫) — `hybrid_analyzer.py`의 폴백 `reason`이 영어라 `explanation`·`AI_JUDGMENT` 증거 카드로 흘러든다. **프론트는 막아뒀다**(#126·#128·#130 — 한글 없는 설명은 증거 카드를 숨기고 설명은 한국어 안내로 대체, `Matched rule:` 같은 내부 접두사는 제거). ✅ 실서버 실측에서 **LLM 트랙이 정상 동작하면 전부 한국어로 온다**는 것을 확인했다 — 영어는 stacking 모델이 확신해 LLM을 건너뛰는 구간에서만 나온다. 그래도 그 구간에 걸리는 문자에서는 판단 근거가 통째로 비어 보인다(`docs/backend-ai-fix-list.md`)
